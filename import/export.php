...
function exportCSV($dir, $limit, $where)
{
                 ...
                 $qc="SELECT s.purchase_price AS sku_price, s.sku, p.price, p.name, p.summary, p.description, p.count, p.url, s.id AS sku_id, p.id, sc.name AS category_name FROM shop_product_skus AS s
				  ...
				  $i = 1;	
				   ...
				   $products[0][8] =  iconv("UTF-8", "Windows-1251",'Суммарно на складах');
				   $products[0][9] =  iconv("UTF-8", "Windows-1251",'Категория');
				   $products[0][10] =  iconv("UTF-8", "Windows-1251",'Изображения');
				  ...
				  $catQuery = mysql_query('SELECT sc.name FROM shop_category_products AS scp
				   ...
				   $products[$i][8] = stockCount($row['count']);
				   $products[$i][9] = iconv("UTF-8", "Windows-1251",$cat);
				   $products[$i][10] = $images;
	 ...
	 exportCSV("../csv/", "", " AND p.category_id != 736 ");
